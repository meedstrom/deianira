...
It is not on any ELPA, but if you have [[https://github.com/raxod502/straight.el][straight.el]], you can install the package like so:
...
You need [[https://gitlab.com/interception/linux/plugins/dual-function-keys][interception-tools]] with its included plugin dual-function-keys.  If your distro does not package it, I'm sorry to say this step may take you some work.  On Arch and Guix it's easy.  As an alternative, if you always use a keyboard with QMK firmware, you could configure that keyboard at the firmware level.

I have not found any alternatives.  In Deianira's youth, I was using the buggy and unmaintained [[https://github.com/alols/xcape][xcape]], and it had a glut of issues: it could work fine for a while and then develop a 2-second latency with keystrokes (even with =nice -20=), whereupon the computer had to be rebooted.  This leads to giving up on Deianira.  Something so fundamental needs to be reliable.

*** What is it for?
Emacs cannot detect the fact that you held down and released Control, Meta or Super, so some software lower in the stack needs to tell Emacs about it.  Thus, we configure dual-function-keys to send separate key events when this happens.  It does not matter which key events will represent this as the user almost never sees them printed in the echo area -- they can be F20, F21, F22 or some such.
...
MAPPINGS:
  ...
  # which keycodes would be recognized as such on Wayland.  On Xorg it should be
...
To test, execute this in some terminal.  After satisfied, you can stop the effect by interrupting the terminal with Ctrl-C.

: sudo nice -n -20 udevmon -c /etc/interception/udevmon.yaml

With the program active, you should expect that pressing Ctrl, Alt or Super in Emacs will yield a message like "<XF86Mail> is undefined".  Make sure you see that sort of message for all three modifier keys, so you know they all work.
...
In addition, though it can be superfluous, ensure that the keysyms exist in your current Xkb keymap table, by customizing =dei-xmodmap-rules=.
...
Most common layouts under xkb do not have Alt or Hyper, so even on X11 you'd use xmodmap to bring them into existence.   In lieu of xmodmap, it should be possible to just load a .xkb file that contains Hyper and Alt.  Basically start by generating a =custom.xkb= with xkbcomp and modify, as per:
...
I don't yet know how to write an on-boot service, but creating =udevmon.yaml= inside the system declaration allows you to run an oneliner at any time:
...
(For those just running GNU Guix as a package manager, follow the Arch instructions instead, substituting =/bin/intercept= for =~/.guix-profile/bin/intercept= or some such.)
...
To homogenize, eval this.  Tip: If you enjoy the homogenized keymaps, you can keep using that part of Deianira without ever turning on deianira-mode.  

#+begin_src elisp
(add-hook 'dei-keymap-found-hook #'dei-homogenize-all-keymaps)
#+end_src

You can check the buffer =*Deianira remaps*= to see what it did.  Note that there is no way to undo.  To get back all your bindings, comment out the snippet and restart Emacs.
...
/That/ is the unique utility of Shift.  It's not much, but it's there.  Know thy reasons for using it.  We could probably generalize this benefit with a massive round of key-chord.el mappings inside each and every hydra, but that's a project I'm not tackling.
...
It's annoying in =describe-keymap= output, but they will be hidden the =which-key= popup, if you apply the fix in [[which-key][Fix which-key]].  It doesn't affect the user, so it's a low priority.
...
These commands speak of one *verb,* the verb "transpose", together with a variety of targets or *objects* "char", "word", "sexp", "line" and so on.  We see a similar pattern for the verb "kill":
