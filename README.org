...
The following variables let you specify keys and commands that should always exit the hydra.  Consult the docstrings.  As with =dei-homogenizing-winners=, experuence will tell you over time how to customize these.
...
- by default, =next-buffer= is under the Control-X-prefix (=C-x <right>=), which is static and unchanging anyway

If you do this under a prefix that changes a lot between major modes, like C-c, the next key can behave unexpectedly:
