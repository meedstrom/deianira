...
A [[https://github.com/abo-abo/hydra][hydra]]-based =which-key= banisher.  In some ways the opposite of [[https://gitlab.com/jjzmajic/hercules.el][hercules.el]] (a "which-key based hydra banisher").  In ancient Greek myth, Hercules slew the monster called Hydra, but was then slain himself by the [[https://en.wikipedia.org/wiki/Deianira][princess Deianira]].
...
- I wanted a modifier-free life, and I decided I don't mind typing more to get that.  Longer key sequences beat modifiers.  *But* as a happy side effect of implementing the vision, I found we need surprisingly little extra typing this way, and sometimes even /less/.
  - I first started to learn Vim, lured in by the promise of "modes instead of Control-chords", but what did I find?  An excessive use of capital letters, about as uncomfortable as Control chords.  So Vim is not chord-free at all.  What a strange oversight in its design.  What's the point?  It's just a version of Emacs that uses Shift instead of Control, and ships some pretty good repeat-maps (i.e. hydras) -- that was Vim's real killer feature all along.  Thus began a glimpse of Deianira: I disagree so much with the idea of having "modes" to help you escape chords, and then typing chords within those modes anyway.
...
- We rely on an external Linux utility, [[https://github.com/kmonad/kmonad][kmonad]], to turn Control, Meta and Super into leader keys, even while they continue to work as modifiers.  But even should the utility stop working somehow, we won't be left dead in the water.  Our mass-remapping is done by the [[https://github.com/meedstrom/massmapper][massmapper]] Emacs-Lisp library, which will always work, so your bindings remain in the location you were expecting.  With a kmonad service outage, you'll just have to do one of two things:
  1. Chord at first to start a key sequence, like a normal Emacser (boo hiss).  Even forgetting about the hydra system, the remaps you've made along the way remain accessible, and the bindings are not worse than before you started.
...
** Can I keep ergoemacs/xah-fly-keys?
In theory... maybe?  I haven't tested them.
